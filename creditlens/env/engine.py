...
class CreditLensEnv:
    ...
    def step(self, action: UnderwritingAction) -> StepResult:
        ...
        if action.action_type == ActionType.REQUEST_INFO:
            ...
            fld = action.params.get("field_name", "income_proof")
            inc = float(applicant_row.get("income", 60000.0))
            employer = applicant_row.get("employer_name", "Verified Employer")
            app_name = applicant_row.get("applicant_name", "Applicant")
            bal = float(applicant_row.get("bank_balance", 12500.0))
            
            if fld == "bank_statements":
                self._last_requested_doc = {
                    "document_type": "Verified 6-Month Bank Statements",
                    "status": "VERIFIED_AUTHENTIC",
                    "summary": f"Average monthly inflow: ${inc/12:,.2f}. Zero NSF/overdraft fees in 180 days. Verified liquid balance: ${bal:,.2f}.",
                    "confidence": "99.4% (Direct Open Banking Sync)",
                }
            elif fld == "tax_returns":
                self._last_requested_doc = {
                    "document_type": "Certified IRS Form 1040 (2-Year Transcripts)",
                    "status": "IRS_TRANSCRIPT_MATCHED",
                    "summary": f"Two-year verified Adjusted Gross Income (AGI): ${inc:,.2f}/yr. No outstanding IRS liens or unpaid balances.",
                    "confidence": "100.0% (IRS IVES Certified)",
                }
            elif fld == "employment_letter":
                self._last_requested_doc = {
                    "document_type": "Employer Verification & Wage Certificate",
                    "status": "HR_VERIFIED",
                    "summary": f"Confirmed by {employer} HR: {applicant_row.get('employment_type', 'Permanent Full-Time')}. Current tenure: {applicant_row.get('employment_years', 2.0):.1f} years.",
                    "confidence": "98.7% (Equifax Work Number)",
                }
            elif fld == "identity_document":
                is_fraud = bool(applicant_row.get("is_fraud", False))
                self._last_requested_doc = {
                    "document_type": "Government Photo ID & Biometric Verification",
                    "status": "FLAGGED_INCONSISTENCY" if is_fraud else "KYC_VERIFIED",
                    "summary": (
                        "CRITICAL ALERT: Biometric facial scan matched known identity theft ring cluster."
                        if is_fraud else
                        f"Government ID confirmed: {applicant_row.get('masked_id')}. Address utility cross-referenced and verified."
                    ),
                    "confidence": "61.2% (Anomaly Detected)" if is_fraud else "99.8% (Biometric Liveness Passed)",
                }
            else:  # income_proof
                self._last_requested_doc = {
                    "document_type": "Certified Paystubs (Recent 30 Days)",
                    "status": "PAYROLL_CERTIFIED",
                    "summary": f"Consecutive semi-monthly paystubs confirm annualized salary of ${inc:,.2f} with regular W-2 tax withholdings.",
                    "confidence": "99.2% (ADP Payroll Integration)",
                }
        else:
            self._last_requested_doc = None
    ...
    def _build_observation(self) -> LoanObservation:
        ...
        return LoanObservation(
            ...
            work_sector=row.get("work_sector"),
            employment_type=row.get("employment_type"),
            layoff_risk=row.get("layoff_risk"),
            bank_balance=float(row["bank_balance"]) if pd.notna(row.get("bank_balance")) else None,
            requested_doc_details=getattr(self, "_last_requested_doc", None),
        )
